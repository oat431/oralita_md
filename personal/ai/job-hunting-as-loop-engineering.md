...
The separation is structural: **the engine has write access to `targets/` and `loop/state/` only.** `profile/` is the frozen anchor - the ground truth the optimizer is forbidden to rewrite ([[graph-engineering]] section 12).
...
- Weekly counter-metric: **% of tailored resumes the user would actually send.** If output volume rises while that falls, the loop is Goodharting itself ([[graph-engineering]] section 12) - tighten the threshold or the prompts.
