---
song: Once Upon a December
source: Anastasia (1997 Animated Film)
character: Anya (Anastasia)
lesson_type: concept
topic: English Through Songs
audience_level: beginner-intermediate
grammar_focus:
  - Simple Present
  - Present Continuous
  - Poetic word order
  - Figurative language
  - Sensory vocabulary
prerequisites:
  - "[05.1 Present Simple](https://github.com/oat431/general-knowledge/tree/main/English%20Skill/02%20Tenses%20%26%20Time/05%2012%20English%20Tense/05.1%20Present%20Simple.md)"
  - "[05.2 Present Continuous](https://github.com/oat431/general-knowledge/tree/main/English%20Skill/02%20Tenses%20%26%20Time/05%2012%20English%20Tense/05.2%20Present%20Continuous.md)"
related:
  - "[30 Collocations](https://github.com/oat431/general-knowledge/tree/main/English%20Skill/09%20Vocabulary%20Building/30%20Collocations.md)"
tags:
  - english
  - song
  - anastasia
  - present-tense
  - figurative-language
  - memory
---

# Once Upon a December
> from Anastasia: sung by Anya

---

## References

tsukimiya. (2013, July 27). *Once Upon a December : Ost. Anastasia*. BlogGang. Retrieved July 20, 2026, from https://www.bloggang.com/m/viewdiary.php?id=lazy-flowers&month=07-2013&date=27&group=4&gblog=216

---

## Learning Objectives
By studying this song, you will be able to:
- Recognize **Simple Present** and **Present Continuous** in poetic context
- Understand **sensory vocabulary** (sight, sound, touch) used to describe memories
- Identify **figurative language** in short, simple lines
- Appreciate how **word order inversion** creates poetic effect in English

---

## Lyric + Thai Translation

### Verse 1

> Dancing bears, painted wings,
> Things I almost remember
> And a song someone sings,
> Once upon a December

> มองลวดลาย แสง และ เส้น เหมือนเคยเห็นเคยจดจำไว้
> ยินเสียงเพลงกล่อมร้องไป
> ครั้งหนึ่งให้อดีตเหมันต์

### Chorus

> Someone holds me safe and warm
> Horses prance through a silver storm
> Figures dancing gracefully
> Across my memory

> แขนโอบอุ้ม บรรจง ประคอง
> ม้าคะนอง โผนกระโจนเรียงกัน
> คนเต้นงามเกินรำพัน
> ซ่อนอยู่ที่ใจฉันนี้

### Verse 2

> Far away, long ago
> Glowing dim as an ember
> Things my heart used to know
> Things it yearns to remember

> ไกลแสนไกลย้อนเวลา
> ภาพติดตาไม่อาจเมินหนี
> สิ่งที่ใจเคยคุ้นดี
> ล้วนมากมีไม่อาจทานไหว

### Chorus (repeat)

> Someone holds me safe and warm
> Horses prance through a silver storm
> Figures dancing gracefully
> Across my memory

> แขนโอบอุ้ม บรรจง ประคอง
> ม้าคะนอง โผนกระโจนเรียงกัน
> คนเต้นงามเกินรำพัน
> ซ่อนอยู่ที่ใจฉันนี้

### Outro

> And a song
> Someone sings
> Once upon a December

> เสียงร้องเพลงก้องสดใส
> ครั้งหนึ่งในอดีตผ่าน....มา

---

## Lyric + Thai Translation: Detail

### Verse 1: ภาพในความทรงจำ

| #   | English                       | Thai                     | หมายเหตุ                                                            |
| --- | ----------------------------- | ------------------------ | ------------------------------------------------------------------- |
| 1   | Dancing bears, painted wings, | มองลวดลาย แสง และ เส้น   | **Reduced clause**: "[There are] dancing bears" — verb ถูกตัดออก    |
| 2   | Things I almost remember      | เหมือนเคยเห็นเคยจดจำไว้  | **Relative clause**: "Things [that] I almost remember"              |
| 3   | And a song someone sings,     | ยินเสียงเพลงกล่อมร้องไป  | **Reduced clause**: "a song [that] someone sings"                   |
| 4   | Once upon a December          | ครั้งหนึ่งให้อดีตเหมันต์ | **Play on "Once upon a time"** = กาลครั้งหนึ่ง (fairy tale opening) |

### Chorus: ภาพเคลื่อนไหวในความทรงจำ

| # | English | Thai | หมายเหตุ |
|---|---------|-----|----------|
| 5 | Someone holds me safe and warm | แขนโอบอุ้ม บรรจง ประคอง | **Sensory**: touch (safe and warm) |
| 6 | Horses prance through a silver storm | ม้าคะนอง โผนกระโจนเรียงกัน | **Metaphor** "silver storm" = หิมะ/แสงระยิบ |
| 7 | Figures dancing gracefully | คนเต้นงามเกินรำพัน | "figures" = รูปร่าง, เงาคน |
| 8 | Across my memory | ซ่อนอยู่ที่ใจฉันนี้ | "across my memory" = ผ่านความทรงจำ |

### Verse 2: อดีตที่เลือนราง

| # | English | Thai | หมายเหตุ |
|---|---------|-----|----------|
| 9 | Far away, long ago | ไกลแสนไกลย้อนเวลา | **Adverb phrases**: ไกลและนานมาแล้ว |
| 10 | Glowing dim as an ember | ภาพติดตาไม่อาจเมินหนี | **Simile** "as an ember" = เหมือนถ่านที่ยังคุ (ไฟเกือบดับ) |
| 11 | Things my heart used to know | สิ่งที่ใจเคยคุ้นดี | **"used to"** = เคย (อดีตที่ไม่ทำแล้ว) |
| 12 | Things it yearns to remember | ล้วนมากมีไม่อาจทานไหว | "yearns" = โหยหา, ปรารถนาอย่างแรงกล้า |

### Outro: เสียงสุดท้าย

| # | English | Thai | หมายเหตุ |
|---|---------|-----|----------|
| 13 | And a song | เสียงร้องเพลงก้องสดใส | Repetition of opening |
| 14 | Someone sings | | **Present Simple** — timeless |
| 15 | Once upon a December | ครั้งหนึ่งในอดีตผ่าน....มา | Returns to the title — circular structure |

---

## Grammar Points

### 1. Reduced Clauses: ประโยคที่ตัดคำออก
**ปรากฏในเพลง:** ท่อน 1, 2, 3

- **Dancing bears, painted wings** = [There are] dancing bears [and] painted wings
- **Things I almost remember** = Things [that/which] I almost remember
- **A song someone sings** = A song [that] someone sings

**รูปแบบ:** ตัด relative pronoun (that/which/who) ออกเมื่อเป็น object

> ⚠️ **Thai Speaker Trap:** ภาษาไทยไม่มี relative pronoun ให้ตัด — แต่ในอังกฤษ "Things I remember" เป็น informal/ poetic ที่ตัด "that" ออก ซึ่งเป็นเรื่องปกติในเพลงและบทกวี

🔗 ดูเพิ่ม: [09 Clauses](https://github.com/oat431/general-knowledge/tree/main/English%20Skill/03%20Sentence%20Building/09%20Clauses.md)

---

### 2. "Used to" — เคย (แต่ไม่ทำแล้ว)
**ปรากฏในเพลง:** ท่อน 11

- Things my heart **used to** know = สิ่งที่หัวใจเคยรู้จัก (แต่ตอนนี้ลืมไปแล้ว)

**รูปแบบ:** used to + base verb

| Form | Meaning | Example |
|------|---------|---------|
| **used to** | เคย (แต่ไม่ทำแล้ว) | I **used to** play piano. |
| **be used to** | คุ้นเคยกับ | I **am used to** waking up early. |
| **get used to** | เริ่มคุ้นเคยกับ | I'm **getting used to** the new job. |

> ⚠️ **Thai Speaker Trap:** "used to" 3 แบบนี้ความหมายต่างกันมาก — "I used to smoke" (เคยสูบแต่เลิกแล้ว) ≠ "I am used to smoking" (คุ้นเคยกับควันบุหรี่)

🔗 ดูเพิ่ม: [05.5 Past Simple](https://github.com/oat431/general-knowledge/tree/main/English%20Skill/02%20Tenses%20%26%20Time/05%2012%20English%20Tense/05.5%20Past%20Simple.md) (used to อยู่ในหมวด past habits)

---

### 3. Simile: เปรียบเทียบโดยใช้ "as" หรือ "like"
**ปรากฏในเพลง:** ท่อน 10

- Glowing dim **as** an ember = ส่องสว่างจางๆ เหมือนถ่านที่ยังคุ

| Simile | Meaning | Thai |
|--------|---------|-----|
| dim **as** an ember | จางเหมือนถ่านคุ | ถ่านที่เกือบดับ — ยังมีแสงอยู่บ้าง |
| soft **as** thunder (เพลงที่ 1) | แผ่วเบาดั่งฟ้าร้อง | ขัดแย้งเพื่อเน้น |
| like objects (เพลงที่ 2) | เหมือนสิ่งของ | ดูถูก |

> 💡 **Why this matters:** Simile ปรากฏในภาษาอังกฤษทุกวัน — "quiet as a mouse," "busy as a bee," "like a house on fire" — ถ้ารู้แค่ literal meaning จะไม่เข้าใจ

---

### 4. Sensory Vocabulary: คำที่เกี่ยวกับประสาทสัมผัส
**ปรากฏในเพลง:** ทั้งเพลง

| Sense | English | Thai | ท่อน |
|-------|---------|-----|------|
| **Sight** | dancing bears, painted wings | หมีเต้น, ปีกวาดสี | Verse 1 |
| **Sight** | figures dancing gracefully | คนเต้นอย่างงดงาม | Chorus |
| **Sight** | glowing dim as an ember | ส่องจางเหมือนถ่านคุ | Verse 2 |
| **Touch** | safe and warm | ปลอดภัยและอบอุ่น | Chorus |
| **Sound** | a song someone sings | เพลงที่ใครร้อง | Verse 1 |

> 💡 **Why this matters:** การอธิบายความทรงจำด้วย sensory details ทำให้ภาษาอังกฤษมีชีวิตชีวา — ไม่ใช่แค่ "I remember" แต่ "I remember the warmth, the music, the dancing figures"

---

## Vocabulary Highlights

### คำศัพท์สำคัญจากเพลง

| English | Thai | Part of Speech | Example from Song |
|---------|------|---------------|-------------------|
| **dancing** | เต้นรำ | verb (present participle) | dancing bears |
| **painted** | วาดสี, ระบายสี | verb (past participle) | painted wings |
| **prance** | กระโดดเดินอย่างสง่า | verb | horses prance |
| **gracefully** | อย่างงดงาม, สง่า | adverb | dancing gracefully |
| **ember** | ถ่านที่ยังคุ | noun | dim as an ember |
| **yearn** | โหยหา, ปรารถนา | verb | yearns to remember |
| **memory** | ความทรงจำ | noun | across my memory |
| **storm** | พายุ | noun | silver storm |
| **figure** | รูปร่าง, เงาคน | noun | figures dancing |

### Collocations: คำที่มักมาคู่กัน

| Collocation | Thai | ตัวอย่างอื่น |
|-------------|------|-------------|
| **safe and warm** | ปลอดภัยและอบอุ่น | The blanket kept me safe and warm. |
| **once upon a time** | กาลครั้งหนึ่ง | Once upon a time, there was a princess. |
| **used to know** | เคยรู้จัก | I used to know the answer. |
| **yearn to remember** | โหยหาที่จะจำ | She yearned to remember her childhood. |
| **far away, long ago** | ไกลแสนไกล นานมาแล้ว | The story happened far away, long ago. |

🔗 ดูเพิ่ม: [30 Collocations](https://github.com/oat431/general-knowledge/tree/main/English%20Skill/09%20Vocabulary%20Building/30%20Collocations.md)

---

## Knowledge Connections

- [05.1 Present Simple](https://github.com/oat431/general-knowledge/tree/main/English%20Skill/02%20Tenses%20%26%20Time/05%2012%20English%20Tense/05.1%20Present%20Simple.md): "Someone sings," "Someone holds me"
- [05.2 Present Continuous](https://github.com/oat431/general-knowledge/tree/main/English%20Skill/02%20Tenses%20%26%20Time/05%2012%20English%20Tense/05.2%20Present%20Continuous.md): "Dancing bears" (present participle as adjective)
- [09 Clauses](https://github.com/oat431/general-knowledge/tree/main/English%20Skill/03%20Sentence%20Building/09%20Clauses.md): Reduced clauses — "Things I remember"
- [30 Collocations](https://github.com/oat431/general-knowledge/tree/main/English%20Skill/09%20Vocabulary%20Building/30%20Collocations.md): "safe and warm," "once upon a time"
- [05.5 Past Simple](https://github.com/oat431/general-knowledge/tree/main/English%20Skill/02%20Tenses%20%26%20Time/05%2012%20English%20Tense/05.5%20Past%20Simple.md): "used to" for past habits

---

## Key Takeaways

1. **Reduced clauses คือ poetic English**: "Things I remember" ตัด "that" ออก — เป็นเรื่องปกติในเพลงและบทกวี ไม่ใช่ error
2. **"Once upon a December" = play on "Once upon a time"**: ใช้โครงสร้าง fairy tale แต่เปลี่ยน "time" เป็น "December" เพื่อเน้นความทรงจำเฉพาะเจาะจง
3. **"Used to" ≠ "be used to"**: "Things my heart used to know" = เคยรู้แต่ลืมแล้ว ≠ คุ้นเคยกับ
4. **Sensory language คือหัวใจของ memory description**: ไม่ใช่แค่ "I remember" แต่ "I remember the warmth, the music, the dancing figures"
5. **"Ember" = ถ่านที่ยังคุ**: ไฟเกือบดับ — เปรียบเทียบความทรงจำที่เลือนรางแต่ยังมีชีวิตอยู่บ้าง
6. **Simile ใช้ "as" หรือ "like"**: "dim as an ember" — โครงสร้างเดียวกับ "soft as thunder" ในเพลงแรก
