---
date: 2026-07-27
tags: [fitness, program, phase-2, barbell]
---

# Phase 2 — Barbell Foundation (Weeks 5-8)

## Body Composition Baseline (Smart Scale — Xiaomi S400)

| Metric | Value | Target (End of Phase 2) |
|--------|-------|--------------------------|
| Weight | 121.1 kg | 118 kg |
| Body fat | 43.1 kg (35.6%) | 39 kg (33%) |
| Muscle mass | 76.0 kg | 75+ kg (preserve) |
| Visceral fat | 23 | <18 |
| Body water | 45.5% | >48% |
| BMR | 2,104 kcal | — |
| Waist-to-hip | 1.1 | <1.05 |

---

## Schedule (Mon / Tue / Wed / Sat)

| Day | Focus | Key Lift | Time |
|-----|-------|----------|------|
| Mon | Upper (Barbell) | Bench Press + OHP | ~60 min |
| Tue | Lower (Barbell) | Back Squat | ~55 min |
| Wed | Run/Walk + Core | Intervals | ~40 min |
| Thu | Rest | — | — |
| Fri | Rest | — | — |
| Sat | Full Body | Deadlift + Squat variant | ~60 min |
| Sun | Rest | — | — |

---

## Calories & Recovery

- **2,200 kcal/day** — kept from W4. Re-evaluate if weight drops >0.6 kg/week for 2 consecutive weeks (bump +200)
- **Protein: 160g minimum** — you've hit this 3 weeks straight. Don't break the streak
- **Water: 3-4L/day** — your body water is 45.5% (low normal). Hydrate more. This affects scale readings and recovery
- **Sleep: 7h+** — you averaged 7.3h in W4. Protect this
- **Weigh-in: every morning** — after bathroom, before food. Track the smart scale metrics every Sunday

---

## Week 5 — Learn the Barbell Movements

### Mon — Upper (Barbell Bench + OHP)

| Exercise            | Sets × Reps | Rest    | W5 Weight | Notes                                                                            |
| ------------------- | ----------- | ------- | --------- | -------------------------------------------------------------------------------- |
| Barbell Bench Press | 3×5         | 2-3 min | **40 kg** | You did this in W2. Start here, add 2.5 kg if easy. Bar touches chest, no bounce |
| Barbell OHP         | 3×5         | 2 min   | **30 kg** | Strict press. No leg drive. Bar starts at collarbone, lock out overhead          |
| Chest-Supported Row | 3×8         | 90 sec  | **25 kg** | All 3 sets same weight                                                           |
| Face Pulls          | 3×12        | 60 sec  | **17 kg** | Keep these — shoulder health                                                     |
| Tricep Pushdown     | 2×12        | 60 sec  | **17 kg** |                                                                                  |

### Tue — Lower (Barbell Squat)

| Exercise           | Sets × Reps | Rest    | W5 Weight | Notes                                                                                                      |
| ------------------ | ----------- | ------- | --------- | ---------------------------------------------------------------------------------------------------------- |
| Barbell Back Squat | 3×5         | 2-3 min | **40 kg** | First time? Do 2 sets with just the bar (20 kg) for form, then load to 40. Depth: thighs parallel or below |
| Leg Press          | 3×8         | 90 sec  | **50 kg** | Increase from 45 kg "Easy." 3×8 (lower reps, higher weight)                                                |
| Lying Leg Curl     | 3×10        | 60 sec  | **25 kg** | Increase from 22 kg                                                                                        |
| Calf Raises        | 3×15        | 60 sec  | **35 kg** | Increase from 30 kg                                                                                        |
| Plank              | 3×30 sec    | 30 sec  | —         | Add a set                                                                                                  |

### Wed — Run/Walk + Core

| Exercise | Sets × Reps | Rest | W5 Target | Notes |
|----------|-------------|------|-----------|-------|
| Run/Walk Intervals | — | — | **32 min** | 3 min jog / 2 min walk × 6 + 2 min warm-up walk |
| Dead Bugs | 2×10/side | 30 sec | — | Slow and controlled |
| Pallof Press | 2×10/side | 30 sec | Cable, light | Anti-rotation core. If no cable, side planks 2×30 sec |
| Dumbbell Curls (optional) | 2×10 | 60 sec | **15 kg** | Only if you have time |
| Dumbbell Lateral Raises | 2×12 | 60 sec | **8 kg** | ⚠️ Try 8 kg with back against wall. Prove form before increasing |

### Sat — Full Body (Deadlift + Squat)

| Exercise | Sets × Reps | Rest | W5 Weight | Notes |
|----------|-------------|------|-----------|-------|
| Barbell Deadlift (conventional) | 1×5 | — | **60 kg** | One working set after warm-ups. This is your first deadlift — focus on flat back, hips don't shoot up. Warm up: 40×5, 50×3, then 60×5 |
| Barbell Front Squat | 3×5 | 2 min | **30 kg** | Lighter squat variant. Bar rests on front delts, elbows high. Teaches upright torso |
| Lat Pulldown | 3×8 | 90 sec | **25 kg** | Increase from 22 kg. Lower reps, higher weight |
| Incline Dumbbell Press | 3×8 | 90 sec | **25 kg** | Increase from 22 kg |
| Reverse Lunges | 3×8/leg | 90 sec | **15 kg** | Stay at 15 kg (was "Hard"). Don't increase until it's "Medium" |
| Seated Cable Row | 3×8 | 60 sec | **32.5 kg** | Increase from 30 kg. Lower reps, higher weight |

---

## Week 6-8 — Linear Progression

**Add 2.5 kg to all barbell lifts each session** if you completed all reps with good form. If a lift stalls (miss reps two sessions in a row), follow the progression rules below.

| Barbell Lift | W5 Start | W6 | W7 | W8 |
|-------------|----------|-----|-----|-----|
| Bench Press | 40 kg | 42.5 | 45 | 47.5 |
| OHP | 30 kg | 32.5 | 35 | 37.5 |
| Back Squat | 40 kg | 42.5 | 45 | 47.5 |
| Deadlift | 60 kg | 65 | 70 | 75 |
| Front Squat | 30 kg | 32.5 | 35 | 37.5 |

Accessories: add 2.5 kg when the current weight feels "Medium" or "Easy."

---

## Run/Walk Progression (Wed)

| Week | Protocol | Total Time |
|------|----------|------------|
| 5 | 3 min jog / 2 min walk × 6 | 32 min (with warm-up) |
| 6 | 4 min jog / 2 min walk × 5 | 30 min |
| 7 | 4 min jog / 2 min walk × 6 | 36 min |
| 8 | 5 min jog / 2 min walk × 4 | 28 min |

Goal by end of Phase 2: continuous 20 min jog without walking. Marathon is still 12+ months away — we're building the aerobic base brick by brick.

---

## Progression Rules

- ✅ Complete all reps with good form → add 2.5 kg next session (barbell) or 2.5 kg when "Easy" (accessories)
- ⚠️ Miss 2+ reps on a lift → repeat same weight next session
- ❌ Miss reps two sessions in a row → drop 10% and build back up
- 🛑 Sharp pain (not muscle burn) → stop, tell me
- 🔄 **New: if deadlift form breaks (rounded back) → drop 10 kg and rebuild.** Deadlift form is non-negotiable

---

## Weekly Tracking

**Smart scale:** Log every Sunday → `phase_2/smart-scale-tracking.md`

**Training log:** Fill after every session → `phase_2/training-log-phase2.md`

---

## Session Protocol

1. **Warm-up (5-8 min):** 5 min cycle/treadmill walk + dynamic stretches + empty bar practice for barbell lifts
2. **Barbell warm-up sets:** For every barbell lift, do 2-3 warm-up sets before the working weight:
   - Set 1: 50% of working weight × 5
   - Set 2: 70% × 3
   - Set 3: 90% × 2
   - Then: working weight × working reps
   - *These don't count toward your 3 working sets and don't need to be logged*
3. **Rest:** 2-3 min on barbell compounds, 90 sec on machine compounds, 60 sec on isolations
4. **Log everything.** Working sets only. Same weight across all working sets.

---

## Recovery Checklist

- [ ] 7+ hours sleep
- [ ] 2,200 kcal with 160g+ protein
- [ ] 3-4L water
- [ ] Smart scale weigh-in every Sunday
- [ ] If soreness prevents full ROM → active recovery, don't force the barbell

---

*Next review: Week 8. Bring your log + smart scale trend data. — Coach*
