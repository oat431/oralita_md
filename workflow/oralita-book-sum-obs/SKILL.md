...
18b. **Sub-agents skip wikilinks entirely (PERSISTENT — verify every batch)** — Different from hallucinated filenames: sub-agents may produce files with ZERO `[[wikilinks]]` because they focus on content and forget the linking requirement. This is NOT a one-off — it happened consistently across ALL batches for the Engineering Foundation vault (7 of 9 sub-agent files had zero wikilinks). **Fix:** After EVERY batch, verify every output file has at least one `[[` link using `grep -c "\[\[" file`. For files missing links, add a `## Related` section with appropriate wikilinks using `patch()` or `execute_code` with file append. **This check should be part of the standard post-batch verification, not just a fix when noticed.** Pattern for batch fix:
```python
import os
base = r'path\to\vault'
related_sections = {
...
25. **Sub-agent language drift (PERSISTENT — check every batch)** — sub-agents can output in unexpected languages (Chinese, Korean, Japanese, etc.) when the source material or their training context is in that language. This is NOT a one-off — it happened 7 times across 3 sessions (CLRS Linear Programming, SWE Theory Design Principles, SWE Theory Project Planning, PLT chapters, AI Machine Learning, AI Reinforcement Learning, AI NLP & Perception). **Likelihood increases when:** source PDF has bilingual content, sub-agent model has strong non-English training data, or the chapter has technical formulas/equations. **Symptoms:** File passes structural verification (YAML, wikilinks, tags) but content is in the wrong language. **Fix:** After EVERY batch, run a language check on ALL output files using Python's explicit Unicode range checks:
```python
c = open(path, encoding='utf-8').read()
chinese = [ch for ch in c if '\u4e00' <= ch <= '\u9fff']
korean = [ch for ch in c if '\uac00' <= ch <= '\ud7af' or '\u3131' <= ch <= '\u318e']
```
If > 0 non-English characters found, rewrite the ENTIRE file in English using `write_file`. Don't try to patch individual lines — the whole file needs rewriting. **This check should be part of the standard post-batch verification, not just a fix when noticed.** The earlier grep `[\\u4e00-\\u9fff]` matched both Chinese AND some ASCII ranges — use Python's explicit Unicode range check for reliable detection.
...
34. **Post-creation file reorganization into subfolders** — when the user says "move files to Part 1 / Part 2 folders" after creation: (a) create subfolders with numbered prefixes (e.g., `01 Physics and Math/`, `02 SWE Process/`), (b) move files using `shutil.move` in `execute_code`, (c) rewrite the Overview with updated `[[subfolder/filename|display]]` wikilinks, (d) fix ALL internal wikilinks in chapter files to use the new subfolder prefix (e.g., `[[01_Dimensions]]` → `[[01 Physics and Math/01_Dimensions|01_Dimensions]]`). Use `execute_code` with a Python script that builds a link map from the file listing and applies replacements. This is faster and more reliable than manual patching. **Key:** the overview wikilinks need the pipe alias (`|display_name`) to keep them readable. Proven on Engineering Foundation vault: 25 files moved to 2 subfolders, 24 internal links fixed in seconds.
